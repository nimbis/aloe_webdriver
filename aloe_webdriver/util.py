    ...
    def wrapped(*args, **kwargs):
        ...
        while True:
            try:
                ...
                retries -= 1
                if retries > 0:
                    sys.stderr.write("retry_on_timeout: Retrying with {} retries remaining".format(retries))
                    continue
                else:
                    sys.stderr.write("retry_on_timeout: Not retrying anymore---allowing exception to pass through")
                    raise
